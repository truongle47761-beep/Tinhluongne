...
function statsHTML(){
 const [a,b,label]=rangeForStats(), list=state.shifts.filter(s=>inRange(s.date,a,b)),t=totals(list),ex=allExtras(list);
 ...
 <div class="big-money">${money(t.total)}</div><div class="small">Tổng nhận</div></section>
 ...
 <section class="section card"><div class="inline"><b>Chi tiết</b><span class="small">${fmtDate(a)} – ${fmtDate(b)}</span></div>
 <div style="margin-top:10px" class="small">Thưởng: <b>${money(ex.thuong||0)}</b> · Phụ cấp: <b>${money(ex.phucap||0)}</b> · Khác: <b>${money(ex.khac||0)}</b></div></section>
 <section class="section"><h3>Danh sách ca</h3>${list.length?`<div class="list">${sortShifts(list).map(shiftCard).join("")}</div>`:`<div class="card empty">Chưa có dữ liệu trong khoảng này.</div>`}</section>`;
}
...
function settingsHTML(){
 return `<section class="section card"><div class="inline"><div><b>Đơn giá mặc định</b><div class="small">Áp dụng khi thêm ca mới</div></div><b>${money(state.settings.defaultRate)}/h</b></div>
